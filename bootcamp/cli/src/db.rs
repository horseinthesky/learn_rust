...
impl JiraDatabase {
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        let story_index = epic
            ...
            .ok_or_else(|| {
                anyhow!("story id not found in epic stories vector")
            })?;
...
impl Database for JSONFileDatabase {
    ...
    fn write_db(&self, db_state: &DBState) -> Result<()> {
        fs::write(
            &self.file_path,
            &serde_json::to_string_pretty(db_state)?,
        )?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn update_epic_status_should_error_if_invalid_epic_id() {
        ...
        let result =
            db.update_epic_status(non_existent_epic_id, Status::Closed);
        assert_eq!(result.is_err(), true);
    }
    ...
    fn update_story_status_should_error_if_invalid_story_id() {
        ...
        let result =
            db.update_story_status(non_existent_story_id, Status::Closed);
        assert_eq!(result.is_err(), true);
    }
